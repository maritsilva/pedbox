...
import { Search, Clock, ChevronRight, Flame, BookOpen, FlaskConical, X, LayoutGrid, GitBranch, ChevronLeft } from 'lucide-react';
          ...
          {showResumos && filteredResumos.length > 0 && (
            <section>
              <div className="flex items-center gap-2 mb-5">
                ...
                <div>
                  <h2 className="text-lg font-extrabold text-foreground">Resumos Clínicos</h2>
                  <p className="text-xs text-muted-foreground">{filteredResumos.length} resumo{filteredResumos.length !== 1 ? 's' : ''} disponíve{filteredResumos.length !== 1 ? 'is' : 'l'}</p>
                </div>
              </div>
          ...
          {showProtocols && filteredProtocols.length > 0 && (
            <section>
              <div className="flex items-center gap-2 mb-5">
                ...
                <div>
                  <h2 className="text-lg font-extrabold text-foreground">Protocolos Clínicos</h2>
                  <p className="text-xs text-muted-foreground">{filteredProtocols.length} protocolo{filteredProtocols.length !== 1 ? 's' : ''} disponíve{filteredProtocols.length !== 1 ? 'is' : 'l'}</p>
                </div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
