...
import { Search, X, Star, Shield, Users, Heart } from 'lucide-react';
    ...
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4 mb-8">
        ...
        <div className="relative md:w-96">
          ...
          <input
            ...
            className="w-full pl-10 pr-24 py-3 bg-white border border-border rounded-xl text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-all"
          />
          <kbd className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] text-muted-foreground bg-secondary border border-border rounded px-1.5 py-0.5 font-mono">CTRL K</kbd>
          {search && (
            <button onClick={() => setSearch('')} className="absolute right-16 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground">
              <X className="w-4 h-4" />
            </button>
          )}
        </div>
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {filtered.map((tool, i) => <ToolCard key={tool.path + tool.label} tool={tool} />)}
        </div>
      )}

      {/* Footer trust bar */}
      <div className="flex flex-wrap justify-center gap-8 mt-12 pt-6 border-t border-border text-xs text-muted-foreground">
        ...
        <span className="flex items-center gap-1.5"><Users className="w-3.5 h-3.5 text-primary" /> Ferramentas pensadas para o plantão pediátrico</span>
