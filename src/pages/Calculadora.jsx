  ...
  if (selectedDrug) {
    ...
    return (
      <div className="max-w-2xl mx-auto px-4 py-8">
        <button onClick={resetDrug} className="flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground mb-6 transition-colors">
        ...
        <div className={`${colors.bg} ${colors.border} border rounded-2xl p-5 mb-6`}>
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className={`text-xs font-semibold ${colors.text}`}>{cat?.label.toUpperCase()}</p>
              <h1 className="text-xl font-bold text-foreground">{selectedDrug.name}</h1>
            </div>
          </div>
        </div>
        ...
        <form onSubmit={handleCalcular} className="flex gap-3 mt-6 mb-4">
          ...
          <button
            type="submit"
            className="bg-primary text-white px-5 py-3 rounded-xl font-semibold flex items-center gap-2 hover:bg-primary/90 transition-colors shadow-sm shadow-primary/30"
          >
